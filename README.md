...
Cryptix network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the
`cryptixd` node specified with the `-s` flag and listen for `addr`
...
file, so on subsequent start ups the `cryptixd` node specified with `-s`
...
* Launch a `cryptixd` node for the DNSSeeder to connect to
...
```bash
git clone https://github.com/cryptix-network/dnsseeder ${GOPATH}/src/github.com/cryptix-network/dnsseeder
cd ${GOPATH}/src/github.com/cryptix-network/dnsseeder
go install .
```
...
`cryptixd` instances, set the following DNS records:
